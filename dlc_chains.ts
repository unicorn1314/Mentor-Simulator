...
export const NEW_CHAIN_EVENTS: GameEvent[] = [
    ...
    // ==========================================
    {
        ...
        category: 'career',
        // 早期触发 (2-8年)
        condition: (s, t, c, f, y) => y >= 2 && y <= 8 && !f['rival_met'] && !f['rival_peace'],
        choices: [
    ...
    // ==========================================
    {
        ...
        category: 'network',
        // Ensure even rejection sets a flag to stop this from appearing again
        condition: (s, t, c, f, y) => s.academic >= 12 && s.resources >= 8 && !f['biz_started'] && !f['biz_rejected'],
        choices: [
            { text: '接受投资，成立公司', description: '你成为了董事长，但也踏入了未知的商海。', effect: () => ({ resources: 5, academic: -2 }), setFlag: 'biz_started' },
            { text: '拒绝，专注学术', description: '投资人骂你死脑筋，但学校表扬了你的纯粹。', effect: () => ({ reputation: 2, academic: 1 }), setFlag: 'biz_rejected' },
            { text: '通过学校技术转移中心转化', description: '流程繁琐，收益少，但胜在安全合规。', effect: () => ({ resources: 2, reputation: 1 }), setFlag: 'biz_rejected' }
        ]
    },
    {
        id: 'ce_biz_2',
    ...
    },
    {
        ...
        choices: [
            { text: '变卖房产，退赔全部所得', description: '倾家荡产，保住了人身自由，但被学校解聘。', effect: () => ({ resources: -20, reputation: -10 }), setFlag: 'biz_failed' },
    ...
    // ==========================================
    {
        ...
        category: 'risk',
        // Check both outcome flags to prevent repetition
        condition: (s, t, c, f, y) => !f['safe_reported'] && !f['safe_hidden'] && y > 3,
        choices: [
            { text: '立即上报保卫处和公安', description: '你展现了极高的原则性，受到了校方表彰，虽然实验室整顿了一周。', effect: () => ({ reputation: 2, academic: 0 }), setFlag: 'safe_reported' },
    ...
    // ==========================================
    {
        ...
        category: 'student', 
        // Check for ignored flag as well
        condition: (s, t, c, f, y) => y >= 3 && y <= 10 && !f['partner_met'] && !f['partner_ignored'],
        choices: [
            { text: '借机搭讪，请喝咖啡', description: '你们相谈甚欢，灵魂契合。', effect: () => ({ satisfaction: 2 }), setFlag: 'partner_met' },
            { text: '礼貌让书，转身离开', description: '科研人不需要爱情。', effect: () => ({ academic: 1 }), setFlag: 'partner_ignored' },
    ...
    },
    {
        ...
        choices: [
            ...
            { text: '带 Ta 一起去实验室', description: '“帮我记录一下数据。” Ta 居然觉得还挺有意思？（极低概率）', effect: () => ({ academic: 1, satisfaction: 1 }), setFlag: 'partner_married' }
        ]
    },
    {
        id: 'ce_love_3',
